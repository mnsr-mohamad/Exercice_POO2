...
List<Location> lo=new ArrayList<>();
        ...
        if(!lo.isEmpty()){
            int choix = Utilitaire.choixListe(lo);
            if(lloc.contains(lo.get(choix-1))){

                lloc.get(choix-1).setDateRestitution(LocalDate.now());
                lloc.get(choix-1).getExemplaire().setDescriptionEtat("griffé++");
                Location lloc2=lo.get(choix-1);
                lloc.remove(lloc2);
            }
        }
